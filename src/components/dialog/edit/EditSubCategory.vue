...
const inputSubCategoryName = ref(props.subcategory.name)
const selectedCategoryId = ref('')

const handleEdit = async () => {
    processing.value = true
    const formData = new FormData()
    formData.append('sub_category_name', inputSubCategoryName.value)
    formData.append('category_name', selectedCategoryId.value)
    formData.append('_method', 'PUT')

    await props.method(formData, props.subcategory.id)
            ...
            <div class="w-full max-w-2xl mx-auto bg-white rounded-xl shadow-lg">
                <div
                    ...
                    <p class="font-medium text-lg">
                        Edit Sub Category {{ props.subcategory.name }}
                    </p>
                    <button
