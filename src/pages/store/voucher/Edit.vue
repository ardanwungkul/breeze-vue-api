...
const voucherId = ref(route.params.id)
const form = ref({
    voucher_name: null,
    voucher_code: null,
...
onBeforeMount(async () => {
    ...
    if (!store.singleVoucher || Object.keys(store.singleVoucher).length === 0) {
        router.replace('/404')
    } else {
        voucher.value = store.singleVoucher

        form.value.voucher_name = voucher.value.voucher_name
        if (voucher.value.voucher_auto_generate == 'false') {
            form.value.voucher_code = voucher.value.code[0].voucher_code
            form.value.voucher_stock = voucher.value.code[0].voucher_stock
        } else {
            form.value.voucher_stock = voucher.value.code.length
        }
        form.value.voucher_validity = voucher.value.voucher_validity
        form.value.voucher_type = voucher.value.voucher_type
        form.value.voucher_condition = voucher.value.voucher_condition
        form.value.voucher_condition_value =
            voucher.value.voucher_condition_value
        form.value.voucher_discount_type = voucher.value.voucher_discount_type
        form.value.voucher_discount_value = voucher.value.voucher_discount_value
        form.value.sub_category_id = voucher.value.subcategory
        form.value.voucher_image_placeholder = voucher.value.voucher_image
            ? import.meta.env.VITE_PUBLIC_BACKEND_URL +
              '/storage/images/voucher/' +
              voucher.value.voucher_image
            : placeholderImage
        form.value.voucher_image_text_color = voucher.value
...
const EditVoucher = async () => {
    ...
    await store.editVoucher(formData, setErrors, processing, voucherId.value)
}
...
<template>
    <StoreLayout title="Edit Voucher">
