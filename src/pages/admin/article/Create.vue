...
onMounted(async () => {
    ...
    data.value.tag = store.create.tag
})
...
const addArticle = async status => {
    ...
    await store.addArticle(formData, setErrors, processing)
}
...
<template>
    <AdminLayout title="Add Articles">
        <Cropper
