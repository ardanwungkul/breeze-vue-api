...
import AddSecondaryBanner from '@/pages/admin/shop/AddSecondaryBanner.vue'
...
const deleteSecondaryBanner = async id => {
    processing.value = true
    await shopStore.deleteSecondaryBanner(id, setErrors, processing)
...
const updateBannerStatus = async (id, event) => {
    ...
    await shopStore.updateSecondaryBannerStatus(formData, setErrors, processing)
    processing.value = false
}
onMounted(async () => {
            ...
            <div
                ...
                <div class="h-10 w-full flex px-3 justify-between items-center">
                    <v-tooltip
                        ...
                        <template v-slot:activator="{ props }">
                            <input
                                ...
                                type="checkbox"
                                name=""
                                :checked="item.secondary_banner_status"
                    ...
                    <v-tooltip
                        :text="
                            item.isDefault
                                ? 'Can\'t Delete Secondary Banner'
                                : 'Delete Secondary Banner'
                        ">
                        <template v-slot:activator="{ props }">
                            <form
                                @submit.prevent="
                                    deleteSecondaryBanner(item.id)
                                ">
                                <button
